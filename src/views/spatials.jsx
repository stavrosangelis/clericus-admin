...
const Spatial = () => {
  ...
  const limit = useSelector((state) => state.spatialsPagination.limit);
  const page = useSelector((state) => state.spatialsPagination.page);
  const orderField = useSelector(
    (state) => state.spatialsPagination.orderField
  );
  const orderDesc = useSelector((state) => state.spatialsPagination.orderDesc);
  ...
  const deleteSelected = async () => {
    ...
    const responseData = await deleteData(`spatials`, data);
    if (responseData) {
      setAllChecked(false);
      setReLoading(true);
    }
