...
const ViewSpatial = lazy(() => import('../components/view-spatial'));
...
class Spatial extends Component {
  ...
  loadReferenceLabelsNTypes() {
    const { spatialEntity } = this.props;
    const { properties } = spatialEntity;
  ...
  render() {
    ...
    const heading = match.params._id === 'new' ? 'Add new spatial' : label;
    ...
    if (!loading) {
      ...
      if (stateRedirectReload) {
        redirectReload = <Redirect to={`/spatial/${newId}`} />;
      }
...
Spatial.propTypes = {
  ...
  spatialEntity: PropTypes.object,
  entitiesLoaded: PropTypes.bool,
  className: PropTypes.string,
};
export default compose(connect(mapStateToProps, []))(Spatial);
