...
function AnnotateTool() {
  const { resourcesTypes } = useSelector((rstate) => rstate);
    ...
    const load = () => {
      const loadData = async () => {
        ...
        if (!unmounted) {
          ...
          if (data !== null) {
            setItem(data);
            const itemResources = data.resources.filter(
  ...
  const contextMenuShow = (e) => {
    ...
    let { clientY: top, clientX: left } = e;
  ...
  const calculateCoordinates = (topParam, leftParam) => {
    ...
    left /= scale;
    const wrapper = document.getElementById('main-wrapper');
    const sideBarOpen = wrapper.classList.contains('nav-open');
    if (sideBarOpen) {
      left += 260;
    }
    return { top, left };
  ...
  const toggleDeleteModal = () => {
    if (!deleteModalVisible) {
      ...
      const refId =
        typeof selectedItem.ref._id !== 'undefined' ? selectedItem.ref._id : '';
      const label =
        typeof selectedItem.ref.label !== 'undefined'
          ? selectedItem.ref.label
          : '';
      const systemType =
        typeof selectedItem.ref.systemType !== 'undefined'
          ? selectedItem.ref.systemType
          : '';
      const description =
        typeof selectedItem.ref.description !== 'undefined'
          ? selectedItem.ref.description
          : '';
      setEditItemState({
        refId,
        label,
  ...
  if (!loading && item !== null) {
    ...
    const { label: deleteLabel = '', refId: deleteId = '' } = editItemState;
