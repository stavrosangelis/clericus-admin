...
const Plot = () => {
  ...
  const _tickFormat = (val) => val.toString();
  ...
  const updateHint = (val) => {
    ...
    const newDate = new Date(year, month, val.x);
    const day = weekday[newDate.getDay()];
    const date = `${day}, ${val.x}-${month + 1}-${year}`;
    const hint = { date, x: val.x, y: val.y };
    setValue(hint);
  };
  if (!loading) {
    content = (
      <FlexibleWidthXYPlot height={300} onMouseLeave={() => setValue(false)}>
        <VerticalGridLines />
        <HorizontalGridLines />
        <XAxis tickValues={ticks} tickFormat={(val) => _tickFormat(val)} />
