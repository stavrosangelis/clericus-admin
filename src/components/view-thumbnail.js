...
export default class ViewClasspiece extends Component {
  constructor(props) {
    ...
    this.thumbnailClasspiece = this.thumbnailClasspiece.bind(this);
  }
  ...
  thumbnailClasspiece() {
    ...
    for (let i=0;i<references.length; i++) {
      let reference = references[i];
      if (reference.ref.systemType==="classpiece" && reference.refType==="isPartOf") {
        classpiece = reference;
      }
    }
    if (classpiece!==null) {
      this.setState({classpieceLabel: classpiece.ref.label})
    }
  }

  componentDidMount() {
    this.thumbnailClasspiece();
  }

  render() {
    let classpieceLabel = this.state.classpieceLabel;
    console.log(classpieceLabel);
    let resource = this.props.resource;
    let thumbnailURL = outputDir+classpieceLabel+"/thumbnails/"+resource.fileName;
    let thumbnailImage = <img src={thumbnailURL} alt={resource.label} className="img-fluid img-thumbnail"/>;
