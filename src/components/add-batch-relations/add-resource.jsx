...
import { getResourceThumbnailURL, refTypesList } from '../../helpers';
...
export default class AddResource extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      visible &&
      !prevProps.visible &&
      typeof refTypes.resource !== 'undefined'
    ) {
  ...
  loadMoreResources() {
    const { listPage, listTotalPages } = this.state;
    const currentPage = listPage;
    const lastPage = listTotalPages;
  ...
  loadDefaultRefType() {
    ...
    this.setState({
      refType: refTypesList(refTypes.resource)[0],
    });
  }
  ...
  render() {
    ...
    if (typeof refTypes.resource !== 'undefined') {
    ...
    return (
      ...
      >
        <ModalHeader toggle={() => this.toggleModal('addResourceModal')}>
          Associate selected items with Resource
        </ModalHeader>
        <ModalBody>
          {errorContainer}
          {selectedItemsOutput}
          ...
          <div
            className="events-list-container resources-list-container"
            ref={this.listRef}
          >
            {list}
          </div>
