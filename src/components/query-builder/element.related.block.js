...
import {
  queryBuildRelatedAdd,
  queryBuildRelatedUpdate,
...
const ElementBlock = (props) => {
  ...
  const updateElementName = (value) => {
    ...
    dispatch(queryBuildRelatedUpdate(rstateName, index, values));
  };
  ...
  const addRow = () => {
    ...
    setBoolean('and');
    dispatch(queryBuildRelatedAdd(rstateName, values));
    return false;
  };
  ...
  const removeRow = () => {
    dispatch(queryBuildRelatedRemove(rstateName, index));
  };
